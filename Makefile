...
PARSER = ssc.y
IR_SRC = IR.cpp
AST_SRC = AST.cpp   # or AST.cpp if you use C++
...
DEBUG_FILE = debug_output.txt  # File to be deleted before running and in clean
...
$(IR_OBJ): $(IR_SRC) IR.h
	$(CXX) $(LLVM_CXXFLAGS) -c $(IR_SRC) -o $(IR_OBJ)

# Build AST.o from AST.c (or AST.cpp if using C++)
$(AST_OBJ): $(AST_SRC) AST.h IR.h
	$(CXX) $(LLVM_CXXFLAGS) -c $(AST_SRC) -o $(AST_OBJ)

# Compile the SSC compiler with LLVM support (including IR.o and AST.o)
$(EXECUTABLE): $(LEX_C) $(PARSER_C) $(IR_OBJ) $(AST_OBJ)
	$(CXX) $(LLVM_CXXFLAGS) -o $(EXECUTABLE) $(PARSER_C) $(LEX_C) $(IR_OBJ) $(AST_OBJ) $(LLVM_LDFLAGS) $(LLVM_LIBS)
...
clean:
	rm -f $(LEX_C) $(PARSER_C) $(PARSER_H) $(EXECUTABLE) $(IR_FILE) $(OPT_IR_FILE) $(ASM_FILE) $(OBJ_FILE) $(BIN_FILE) $(IR_OBJ) $(AST_OBJ) $(DEBUG_FILE)
